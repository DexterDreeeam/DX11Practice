...
bool MouseDemo::LoadContent()
{
    ...
    d3dResult = d3dDevice_->CreateBuffer(
        &vertexDesc,
        &resourceData,
        &vertexBuffer_
    );
...
{
    if (customColorVS_)
        customColorVS_->Release();
    if (customColorPS_)
        customColorPS_->Release();
    if (inputLayout_)
...
{
    keyboardDevice_->GetDeviceState(
        sizeof(keyboardKeys_),
        (LPVOID)&keyboardKeys_
    );
    mouseDevice_->GetDeviceState(
        sizeof(mouseState_),
        (LPVOID)&mouseState_
    );
    ...
    if (
        KEYDOWN(prevKeyboardKeys_, DIK_DOWN)
    ...
    if (
        KEYDOWN(prevKeyboardKeys_, DIK_UP)
    ...
    if(
        BUTTONDOWN(mouseState_, 0)
        && !BUTTONDOWN(prevMouseState_, 0)
    )
    {
        ++selectedColor_;
    }
    ...
    if (
        BUTTONDOWN(mouseState_, 1)
        && !BUTTONDOWN(prevMouseState_, 1)
    )
    {
        --selectedColor_;
    }
